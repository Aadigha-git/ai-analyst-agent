...
| `run_sql(query: str)` | `query` — a single SELECT statement | Rows (capped), row count, execution time | Rejects any non-SELECT statement; enforces row limit (default 500) and timeout (default 10s) |
